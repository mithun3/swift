...
-Djava.nio.channels.spi.SelectorProvider=sun.nio.ch.EPollSelectorProvider"  # Linux only; scripts/start.sh applies this conditionally via `uname`

java $JVM_OPTS -cp target/serv-c-1.0.0-SNAPSHOT.jar:target/dependency/* com.fx.persistence.PersistenceMain
```
...
| `BatchPersistenceEngine.MAX_BATCH` | 32,768 | **8,192** | Bounds a single H2 `executeBatch()`/`commit()` to ~47 ms of events at 175K evt/sec, preventing multi-second commits as `fx_trades` grows past 1M rows (MVStore B-tree depth increases) |
