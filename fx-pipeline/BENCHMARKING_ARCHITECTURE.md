...
The `.hlog` output from `TelemetryRecorder` is an HdrHistogram log file. Use the provided Python script to generate a Percentile vs. Latency chart:

```bash
python3 scripts/plot_latency.py /tmp/fx-latency.hlog
```
...
# 5. Visualise the latency histogram
python3 scripts/plot_latency.py /tmp/fx-latency.hlog
```
