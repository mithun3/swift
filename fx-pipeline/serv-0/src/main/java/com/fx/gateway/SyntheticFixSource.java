...
/**
 * {@code SyntheticFixSource} — Deterministic FIX 4.4 message generator for
 * testing.
 *
 * <h2>Purpose</h2>
 * <p>
 ...
 * pre-allocated byte buffer. It enables full pipeline testing with no TCP
 * socket,
 ...
 * </pre>
 * 
 * Fields are delimited by SOH (0x01). The message is written into the
 * pre-allocated
 * {@code byte[]} buffer on every {@link #poll} call without creating any new
 * objects.
 *
...
public final class SyntheticFixSource implements GatewayEventLoop.FixMessageSource {
    ...
    /**
     * Maximum FIX message size: 256 bytes is sufficient for a minimal
     * NewOrderSingle.
     ...
     *                      after this count, {@link #isExhausted()} returns
     *                      {@code true}
     */
    ...
    /**
     * Fills {@code buf} with a synthetic FIX NewOrderSingle message if messages
     * remain.
     *
     * <p>
     * The message is written byte-by-byte into the pre-allocated buffer using
     ...
     *
     * <p>
     * Tag is written as two literal ASCII bytes (both provided by the caller as
     * pre-computed constants) to avoid String indexing overhead.
     *
     * @param buf   destination buffer
     * @param pos   current write position
     * @param tag0  first ASCII byte of the 2-digit tag number (e.g., '3' for tag
     *              35)
     * @param tag1  second ASCII byte of the 2-digit tag number (e.g., '5' for tag
     *              35)
    ...
    /**
     * Writes a tag prefix ({@code tag0}{@code tag1}{@code =}) into {@code buf}
     * without SOH.
     *
     * <p>
     * Used when the value is written separately (e.g., for the sequence number
    ...
    /**
     * Writes a non-negative {@code long} as ASCII decimal digits directly into
     * {@code buf}.
     *
     * <p>
     * This method is the allocation-free replacement for {@code Long.toString()}.
     * It uses a classic digit-extraction technique: repeatedly divides by 10 and
     * writes
     * each digit from the rightmost position. The result is then reversed in-place.
     * No objects are created — all operations are on the pre-allocated
     * {@code byte[]}.
     *
     * @param buf   destination buffer
     * @param pos   current write position
